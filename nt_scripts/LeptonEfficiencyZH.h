#ifndef LeptonEfficiencyZH_h
#define LeptonEfficiencyZH_h

class LeptonEfficiencyZH
{
 public:
  LeptonEfficiencyZH(int era=2012) : era_(era) { }
    ~LeptonEfficiencyZH() {}
	...
	if(era_==2012)
	  {
		...
		if(fabs(eta)<0.9)
		...
		else if(fabs(eta)<2.1)
		  {
		    ...
		    else if(pt<300) { eff.first=1.016*1.002; eff.second=0.002; }
		    else            { eff.first=1.000*1.000; eff.second=0.450; }
		  }
		else
		  {
		    ...
		    eff.second=0.1;
              ...
              eff.second = sqrt(eff.second*eff.second+0.02*0.02);
	  }
	else if(era_==2011)
	  {
	    if(abs(id)==11) eff.second = 0.030/sqrt(2.0);
	    if(abs(id)==13) eff.second = 0.035/sqrt(2.0);
	  }
